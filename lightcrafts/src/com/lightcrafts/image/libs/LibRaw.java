...
public class LibRaw implements
    ...
    // Phase One
    int    ormat, key_off, t_black, black_off, split_col, tag_21a;
    float  tag_210;
...
/* reserved */
    ...
    public String getCameraMake(boolean includeModel) {
        ...
        if ( MODEL.contains( MAKE ) || MAKE.contains( MODEL.substring( 0, MODEL.indexOf( ' ' )) ) )
            return model;
        return make + ' ' + model;
    }
    ...
    private synchronized RenderedImage getImage(boolean halfSize) throws BadImageFileException {
        ...
        final int bands = four_colors ? 3 : 1;
        ...
        final WritableRaster raster = Raster.createInterleavedRaster(
            buf, width, height,
            bands * width, bands,
            bands == 3 ? new int[]{ 0, 1, 2 } : new int[]{0}, null
        );
